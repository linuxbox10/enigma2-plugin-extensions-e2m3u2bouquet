...
class IPTVSetup():
    ...
    def process_provider(self, provider, username, password):
        ...
        for line in PROVIDERS:
            ...
            if PROVIDERS[line]['name'].upper() == provider.upper():
                ...
                return (PROVIDERS[line]['m3u'].replace('USERNAME', username).replace('PASSWORD', password), PROVIDERS[line]['epg'].replace('USERNAME', username).replace('PASSWORD', password), supported_providers)
    ...
    def get_mapping_file(self):
        mapping_file = None
        search_path = [os.path.join(CFGPATH, 'e2m3u2bouquet-sort-override.xml'), os.path.join(os.getcwd(), 'e2m3u2bouquet-sort-override.xml')]
...
def main(argv = None):
    ...
    if uninstall:
        ...
        if bouquet_download and not bouquet_url:
            ...
            if pos != -1:
                bouquet_url = m3uurl[0:pos + 7] + '?username={}&password={}&type=dreambox&output=ts'.format(username, password)
        panel_bouquet = None
        if bouquet_url:
