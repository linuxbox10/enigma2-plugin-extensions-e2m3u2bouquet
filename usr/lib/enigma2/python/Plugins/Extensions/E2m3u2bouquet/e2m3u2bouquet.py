...
class IPTVSetup():
        ...
        with open(os.path.join(ENIGMAPATH, 'bouquets.tv'), 'r') as f:
            for line in f:
                if line.startswith('#NAME'):
                    continue
                elif '.suls_iptv_{}'.format(self.get_safe_filename(providername)) not in line:
    ...
    def save_bouquet_entry(self, f, channel):
        ...
        """
        f.write('#SERVICE {}:{}:{}\n'.format(channel['serviceRef'], urllib.quote(channel['stream-url']), self.get_service_title(channel).encode('utf-8')))
                        ...
                        if not cat.startswith('VOD'):
                            ...
                            for x in dictchannels[cat]:
                                ...
                                if x['enabled']:
                                    ...
                                    f.write('{}<channel id="{}">{}:http%3a//example.m3u8</channel> <!-- {} -->\n'.format(indent, self.xml_escape(tvg_id.encode('utf-8')), epg_service_ref, self.xml_safe_comment(self.xml_escape(self.get_service_title(x).encode('utf-8')))))
    ...
    def process_provider(self, provider, username, password):
        ...
        for line in PROVIDERS:
            ...
            if PROVIDERS[line]['name'].upper() == provider.upper():
                ...
                return (PROVIDERS[line]['m3u'].replace('USERNAME', urllib.quote_plus(username)).replace('PASSWORD', urllib.quote_plus(password)), PROVIDERS[line]['epg'].replace('USERNAME', urllib.quote_plus(username)).replace('PASSWORD', urllib.quote_plus(password)), supported_providers)
    ...
    def get_mapping_file(self, providername):
        mapping_file = None
        search_path = [os.path.join(CFGPATH, self.get_safe_filename(providername) + '-sort-override.xml'), os.path.join(os.getcwd(), self.get_safe_filename(providername) + '-sort-override.xml')]
...
def main(argv = None):
    ...
    if uninstall:
        ...
        if bouquet_download and not bouquet_url:
            ...
            if pos != -1:
                bouquet_url = m3uurl[0:pos + 7] + '?username={}&password={}&type=dreambox&output=ts'.format(urllib.quote_plus(username), urllib.quote_plus(password))
        panel_bouquet = None
        if bouquet_url:
        ...
        categoryorder, category_options, dictchannels = e2m3uSetup.parse_m3u(m3ufile, iptvtypes, sttv, stvod, panel_bouquet, xcludesref, provider)
